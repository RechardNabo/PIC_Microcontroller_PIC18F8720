...
void main(void) {
    ...
    unsigned char data, H_data, S_data;
    ...
    TRISD = 0x00;
    while(TRUE){
        H_data = EEPROM_RD(0x00);
        //EEPROM_WR(0x00,(float)ADC_Read(0));__delay_ms(50);
        
        LCD_20_LINE1();
        LCD_DISPLAY("*PIC18F8720-EEPROM*");
        
        
        PORTD = H_data;
        LCD_20_LINE2();
        data = (float)ADC_Read(0);
        if(data > H_data){
            EEPROM_WR(0x00,data);__delay_ms(50);
            H_data = data;
        }
        LCD_DISPLAY("Saved Data : ");
        LCD_DISPLAY_HEX((float)H_data);
        LCD_20_LINE3();
        LCD_DISPLAY("Current Data: ");
        LCD_DISPLAY_HEX((float)ADC_Read(0));
        LCD_20_LINE4();
        LCD_DISPLAY("Address : ");
